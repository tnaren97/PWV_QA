...
%             ...
% Change folder to patient folder directory
...
folder = 'life_life00040_11372_2021-01-13-h08\dicoms';
result_dir = 'D:\PWV\PWV_data\results\';
num = '08';
pwvnum = '07';
anat = 'A';
...
if strcmp(anat, 'A')
    z = 2;
    asc_name = sprintf('bssfp_masks\\%s_bssfp_AscAo.mat',id);
    AscAof = [result_dir asc_name];
    desc_name  = sprintf('bssfp_masks\\%s_bssfp_DescAo.mat',id);
    DescAof = [result_dir desc_name];   
    ...
    z = 1;
    abd_name  = sprintf('bssfp_masks\\%s_bssfp_AbdAo.mat',id);
    AbdAof = [results_dir abd_name];
...
img_name  = sprintf('bssfp_masks\\%s_images.mat',id);
bssfpf = [result_dir img_name];
bssfp = zeros(256,256,length(dirFiles));
lifeDir = [home_dir '\' folder '\' scan];
cd(lifeDir);
untar([scan '.tgz'])
dirFiles = dir('*.dcm');
