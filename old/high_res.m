...
%     crop = [190 190+cropSize-1 195 195+cropSize-1];
    crop = [105 105+cropSize-1 131 131+cropSize-1];
%   radrange = [6 25];
    radrange = [10 35]; %radius range for Hough Transform
...
%             if ~any(any(BW)) || (n==2 && isequal(BWold, BW))
...
%         circ1 = circleroi(55, 40, 22, cropSize);
%         circ2 = circleroi(95, 100, 18, cropSize);
%         BW = circ1 + circ2;
% 
%         if ~nnz(BW)
...
AscAo_mask(:,:,1:sys_beg-1) = repmat(circ1, 1, 1, sys_beg-1);
DescAo_mask(:,:,1:sys_beg-1) = repmat(circ2, 1, 1, sys_beg-1);
AscAo_mask(:,:,sys_mid+1:end) = repmat(circ1, 1, 1, bssfpFrames-sys_mid);
DescAo_mask(:,:,sys_mid+1:end) = repmat(circ2, 1, 1, bssfpFrames-sys_mid);
...
DescAo_flow = circshift(AscAo_flow,7);
DescAo_area = circshift(AscAo_area,7);
if numROIs==2
...
%     in2 = inpolygon(DescAo_area(in),DescAo_flow(in),free2.Position(:,1),free2.Position(:,2));
    ...
    plot(xq,yq); 
    str = ['Slope = ' num2str(coef(1)) '\newlineIntcpt = ' num2str(coef(2))];
    text(min(DescAo_area_int)+0.1,max(DescAo_flow)+0.1,str);
    hold off;
    DescAo_PWV = coef(1);
    disp(['PWV_QA = ' num2str(coef(1)*0.01) ' m/s']);
    mkdir('DescAo');
