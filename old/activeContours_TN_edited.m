...
%     early_sys = inpolygon(AscAo_area(systole),AscAo_flow(systole),free2.Position(:,1),free2.Position(:,2));
    figure; 
    scatter([AscAo_area(3*end/4+1:end) AscAo_area(1:end/4)], [AscAo_flow(3*end/4+1:end) AscAo_flow(1:end/4)],[], linspace(1,length(AscAo_area)/2,length(AscAo_area)/2))
    colormap jet
    % colorbar
    ...
    text(max(AscAo_area)-0.5,max(AscAo_flow)-0.5,str);
    hold off;
    AscAo_PWV = coef(1);
...
%     early_sys = inpolygon(AscAo_area(systole),AscAo_flow(systole),free2.Position(:,1),free2.Position(:,2));
    figure; 
    scatter([DescAo_area(3*end/4+1:end) DescAo_area(1:end/4)], [DescAo_flow(3*end/4+1:end) DescAo_flow(1:end/4)],[], linspace(1,length(DescAo_area)/2,length(DescAo_area)/2))
    colormap jet
    % colorbar
...
%     in2 = inpolygon(DescAo_area(in),DescAo_flow(in),free2.Position(:,1),free2.Position(:,2));
    ...
    plot(xq,yq); 
    str = ['Slope = ' num2str(coef(1)) '\newlineIntcpt = ' num2str(coef(2))];
    text(max(DescAo_area)-0.5,max(DescAo_flow)-0.5,str);
    hold off;
    DescAo_PWV = coef(1);
    disp(['PWV_QA = ' num2str(coef(1)*0.01) ' m/s']);
    mkdir('DescAo');
...
%     early_sys = inpolygon(AscAo_area(systole),AscAo_flow(systole),free2.Position(:,1),free2.Position(:,2));
    figure; 
    scatter([AbdAo_area(3*end/4+1:end) AbdAo_area(1:end/4)], [AbdAo_flow(3*end/4+1:end) AbdAo_flow(1:end/4)],[], linspace(1,length(AbdAo_area)/2,length(AbdAo_area)/2))
    colormap jet
    % colorbar
...
%     in2 = inpolygon(AbdAo_area(in),AbdAo_flow(in),free2.Position(:,1),free2.Position(:,2));
    ...
    plot(xq,yq); 
    str = ['Slope = ' num2str(coef(1)) '\newlineIntcpt = ' num2str(coef(2))];
    text(max(AbdAo_area)-0.5,max(AbdAo_flow)-0.5,str);
    hold off;
    AbdAo_PWV = coef(1);
    disp(['PWV_QA = ' num2str(coef(1)*0.01) ' m/s']);
    mkdir('AbdAo');
